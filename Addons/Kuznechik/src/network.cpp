...
QString SignalCrosser::DoCommand(KNPCommand KNPcommand, int client_id, QTcpSocket *Client)
{
	...
	if (command == QString::fromUtf8("вперед")) {
		...
		if (shag != kuznecObj->Fstep) {
			return QString::fromUtf8("ERROR,Плохой размер прыжка вперед:допустимо ") + QString::number(kuznecObj->Fstep) + ".\n";
		}
		if (!kuznecObj->canFwd()) {
	...
	if (command == QString::fromUtf8("назад")) {
		...
		int shag = KNPcommand.arguments[0].toInt();
		if (shag != kuznecObj->Bstep) {
			return QString::fromUtf8("ERROR,Плохой размер шага назад:допустимо ") + QString::number(kuznecObj->Bstep) + ".\n";
		}
