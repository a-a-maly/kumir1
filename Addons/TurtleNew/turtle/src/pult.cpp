...
void MainButton::paintEvent(QPaintEvent * event)
{
	...
	if (!downFlag) {
		...
		if (icon) {
			painter.drawImage(1, 1, buttonIcon);
		}
...
TurtlePult::TurtlePult(QWidget* parent, Qt::WindowFlags fl) : QWidget(parent, fl), Ui::TurtlePult()
{
	autoClose = false;
	setupUi(this);
	libMode = false;
	...
	if (!buttRad->loadIcon(":/img/tailup.png")) {
