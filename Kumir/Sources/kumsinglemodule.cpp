...
{
	if (!ready) {
		qDebug("Proga_Value_Line:Module Not Ready.");
	}
	if (type != kumIsp) {
		qDebug("Proga_Value_Line:type!=kumIsp.");
	}
...
{
	if (ready) {
		qDebug("setInstrument():set 2 times.");
	}
...
{
	if (!ready) {
		qDebug("Module:instrument:not ready!");
	}
	if (type != internal) {
		return  NULL;
	}
